...
async fn main() -> Result<(), Error> {
    ...
    let app = Router::new()
        .route("/Prod/axum/", get(root))
        .route("/Prod/axum/foo", get(get_foo).post(post_foo))
        .route("/Prod/axum/foo/:name", post(post_foo_name))
        .route("/Prod/axum/health/", get(health_check))
        // .route("/Prod/axum/add/:queue_name/:name", post(add_message))
